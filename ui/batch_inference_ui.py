...
class PredictThread(QThread):
    ...
    def __init__(self, file_path: str, return_csv: bool = False, parent=None):
        super().__init__(parent)
        self.file_path = file_path
        self.return_csv = return_csv
        self._url = "http://127.0.0.1:5000/batch_predict"

    def run(self):
        try:
            # Basic safety: check file exists
            if not os.path.isfile(self.file_path):
                self.finished_signal.emit({"error": f"file not found: {self.file_path}"})
                return

            with open(self.file_path, "rb") as fh:
                # files: ('filename', fileobj, 'content-type')
                files = {"file": ("upload.csv", fh, "text/csv")}
                data = {}
                if self.return_csv:
                    data["return_csv"] = "true"
                # increase timeout for larger CSVs
                resp = requests.post(self._url, files=files, data=data, timeout=180)
                ...
                elif "text/csv" in ctype or "application/octet-stream" in ctype:
                    # CSV returned — save to disk and return path (or emit bytes)
                    out_path = "predictions_batch.csv"
                    with open(out_path, "wb") as outfh:
                        outfh.write(resp.content)
                    self.finished_signal.emit({"saved_csv": out_path})
                else:
                    # fallback: try JSON parse, otherwise return raw text
                    try:
                        self.finished_signal.emit(resp.json())
                    except Exception:
                        self.finished_signal.emit({"raw": resp.text})
...
class BatchInferenceWidget(QtWidgets.QWidget, Ui_Form):
    ...
    def predict(self):
        ...
        # Create the thread with the file path and optional flag whether to download CSV
        try:
            self._predict_thread = PredictThread(file_path=filename, return_csv=False)
    ...
    def on_prediction_result(self, result):
        ...
        if isinstance(result, dict) and result.get("error"):
            self.label_3.setText(f"Error during prediction:\n{result.get('error')}")
            self.label_3.setVisible(True)
            return
